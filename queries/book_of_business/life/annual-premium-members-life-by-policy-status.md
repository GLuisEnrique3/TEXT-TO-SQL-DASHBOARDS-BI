---
name: annual-premium-members-life-by-policy-status
description: Prima anual y miembros totales de Life (año en curso), desglosado por estado de póliza (ps.Status)
domain: life
tables:
  - claro_bi.BOB_TD
  - claro_bi.dim_cslb
...
tags: [premium, members, life, bob, by-status]
---

# Annual Premium / Members — Life por Estado de Póliza

## Cuándo usar esta consulta

Misma métrica y mismas reglas de filtrado que `annual-premium-members-life-by-agency.md`,
pero desglosada por estado de póliza (`ps.Status`, vía `dim_policy_status`) en vez de por
agencia. Como la base ya excluye `NR` y `Terminated`, el desglose solo mostrará los demás
estados "activos".
...
- Distinta de `annual-premium-members-life-by-risk-policies.md`, que en vez de excluir
  `NR`/`Terminated` filtra explícitamente por estados de riesgo.
