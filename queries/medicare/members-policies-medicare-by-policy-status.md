---
name: members-policies-medicare-by-policy-status
description: Miembros y pólizas de Medicare (año en curso), desglosado por estado de póliza (ps.Status)
domain: medicare
tables:
  - claro_bi.BOB_TD
  - claro_bi.dim_cslb
...
tags: [members, policies, medicare, bob, by-status]
---

# Members / Policies Numbers — Medicare por Estado de Póliza

## Cuándo usar esta consulta

Misma métrica y mismas reglas de filtrado que `members-policies-medicare.md`, pero
desglosada por estado de póliza (`ps.Status`, vía `dim_policy_status`) en vez de un total
único. Como la base ya excluye `NR` y `Terminated`, el desglose solo mostrará los demás
estados "activos".
...
- Distinta de `members-policies-medicare-by-risk-policies.md`, que en vez de excluir
